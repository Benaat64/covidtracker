...
const findPlayerRouter = require('./routes/summoner');
...
loadPlayers();
app.use('/player', findPlayerRouter);
...
app.post('/add_player', async (req, res) => {
    ...
    try {
        const accountResponse = await axios.get(`https://europe.api.riotgames.com/riot/account/v1/accounts/by-riot-id/${gameName}/${tagLine}`, {
            headers: { 'X-Riot-Token': RIOT_API_KEY }
        });

        const puuid = accountResponse.data.puuid;

        const summonerResponse = await axios.get(`https://europe.api.riotgames.com/lol/summoner/v4/summoners/by-puuid/${puuid}`, {
            headers: { 'X-Riot-Token': RIOT_API_KEY }
        });

        const encryptedSummonerId = summonerResponse.data.id;

        const leagueResponse = await axios.get(`https://europe.api.riotgames.com/lol/league/v4/entries/by-summoner/${encryptedSummonerId}`, {
            headers: { 'X-Riot-Token': RIOT_API_KEY }
        });
